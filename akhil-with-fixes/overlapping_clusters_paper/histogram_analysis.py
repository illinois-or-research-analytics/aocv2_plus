...
def main():
    ...
    for k in [10, 20, 30, 40, 50]:
        clusters = clustering_to_dict(
            "/shared/aj_manuscript_data/experiment_0/IKC_"
            + str(k)
            + "_realignment.clustering"
        )
    ...
    for file in os.listdir("/shared/aj_manuscript_data/experiment_1"):
        if file.endswith(".clustering"):
            clusters = clustering_to_dict(
                "/shared/aj_manuscript_data/experiment_1/" + str(file)
            )
    ...
    for file in os.listdir("/shared/aj_manuscript_data/experiment_2"):
        if file.endswith(".clustering"):
            clusters = clustering_to_dict(
                "/shared/aj_manuscript_data/experiment_2/" + str(file)
            )
            k_clusters[file.split(".")[0]] = set.union(*clusters.values())
    ...
    for file in os.listdir("/shared/aj_manuscript_data/experiment_3"):
        if file.endswith(".clustering"):
            clusters = clustering_to_dict(
                "/shared/aj_manuscript_data/experiment_3/" + str(file)
            )
    ...
    for k in [10, 20, 30, 40, 50]:
        ...
        for node in candidates:
            num_nodes_original_cluster = len(
                original_clusters[k].intersection(node_info["citing_neighbors"][node])
            )
            ...
            if total_nodes_network > 0:
                ...
                info[k]["proportion"].append(
                    float(num_nodes_original_cluster / total_nodes_network)
                )
    ...
    for file in os.listdir("/shared/aj_manuscript_data/experiment_1"):
        if file.endswith(".clustering"):
            ...
            for node in candidates:
                num_nodes_original_cluster = len(
                    k_clusters[file.split(".")[0]].intersection(
                        node_info["citing_neighbors"][node]
                    )
                )
                total_nodes_network = len(node_info["citing_neighbors"][node])
                if total_nodes_network > 0:
  ...
  """
    ...
    for k in [10, 20, 30, 40, 50]:
        ...
        ax = sns.boxplot(
            data=info_df[k], x="name", y="proportion", orient="v", width=0.5
        )
