...
def main():
    ...
    for file in os.listdir("./erdos_renyi_graphs"):
        ...
        f = file.split(".")[0]
        plt.savefig(
            "./erdos_renyi_indegree_distribution/" + str(f) + "_distribution.png"
        )
    ...
    for file in os.listdir("./erdos_renyi_ikc_clustering"):
        ...
        print(
            len(
                [
                    k
                    for k in clusters["Full Clusters"].keys()
                    if len(clusters["Full Clusters"][k]) > 1
                ]
            )
        )
...
def run_ikc(network_file, output_path, k):
    ...
    command = (
        "pipenv run python3 modified_IKC.py -e "
        + str(network_file)
        + " -o "
        + str(output_path)
        + "k_"
