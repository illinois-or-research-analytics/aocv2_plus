...
def main():
    ...
    for k in files:
        overlapping2 = (
            "./experiment_2/IKC_" + str(k) + "_km_indegree_1percent.clustering"
        )
        overlapping = (
            "./experiment_1/IKC_" + str(k) + "_km_totaldegree_1percent.clustering"
        )
        ...
        for key_ in clusters.keys():
            if len(clusters[key_]) > 1:
                print(
                    key_,
                    len(clusters[key_]),
                    len(overlapping_clusters[key_]),
                    len(overlapping_clusters2[key_]),
                )
    return 0
