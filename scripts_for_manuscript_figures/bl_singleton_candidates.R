# modified on Aug 21, 2022 to use new Baqiao Liu datae
# George Chacko

rm(list=ls())
setwd('/shared/bl_aoc_redo/aug-20')
library(data.table); library(xtable); library(ggplot2)

# singleton_k <- fread('../aj_manuscript_data/tiervclustercount_singleton_k.csv')
singleton_k <- fread('ikc10_top1_10.k.cluster')

# singleton_m <- fread('../aj_manuscript_data/tiervclustercount_singleton_mcd.csv')
singleton_m <- fread('ikc10_top1_10.mcd.cluster')

ikc10 <- fread('/shared/aj_manuscript_data/experiment_0/IKC_10_realignment.clustering')

ikc10_counts <- ikc10[,.N,by='V1'][,.(V1,N)]
singleton_m_counts <- singleton_m[,.N,by='V1']
singleton_k_counts <- singleton_k[,.N,by='V1']

t1 <- merge(ikc10_counts,singleton_m_counts,by.x='V1',by.y='V1')
t2 <- merge(t1,singleton_k_counts,by.x='V1',by.y='V1')
colnames(t2) <- c('cluster_id','ikc10','aoc_m','aoc_k')

setwd('/shared/aocv2_plus/gc')
fwrite(t2,file='bl_singleton_one_percent_aoc.csv')
