...
## modified on Aug 21 to reflect Baqiao's new data and run on valhalla.cs.illinois.edu
...
k10 <- fread('/shared/aj_manuscript_data/experiment_0/IKC_10_realignment.clustering')
k10_k <- fread('/shared/bl_aoc_redo/aug-19/correct_ikc_10_redo_k.k.cluster')
k10_m <- fread('/shared/bl_aoc_redo/aug-19/correct_ikc_10_redo_k.mcd.cluster')
...
# Since comps2[perc >0,unique(r_id)] only returns rows 1-5 truncate
setwd('/shared/aocv2_plus/gc')

pdf('bl_marker_comps.pdf')
...
pdf('bl_marker_comps_wide.pdf',h=3,w=6)
